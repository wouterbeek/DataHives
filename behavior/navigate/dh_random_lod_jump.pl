:- module(
  dh_random_lod_jump,
  [
    dh_random_lod_jump/2 % -DirectedTriple:compound
                         % +Options:list(nvpair)
  ]
).
...
%! dh_random_lod_jump(-DirectedTriple:compound, +Options:list(nvpair)) is det.

dh_random_lod_jump(DirectedTriple, Options):-
  dh_jump(dh_random_jump, DirectedTriple, Options).
